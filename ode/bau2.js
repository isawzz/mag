...
async function lacunaPresent() {
  ...
  await loadStarImages();
	...
	//this is setup
  let [n, nTypes] = [1, 7]; //types soll < 9 sein
  let fenPoints = lacunaGenerateFenPoints(n, nTypes, 1000, 1000, 0.7); logMinMax(fenPoints);

	//this is present
  B = {};
  let d1 = mDom(document.body,{hline:0,margin:0},{html:'&nbsp;'});
  let [w, h, margin, padding, border] = [500, 500, 0, 0, 7]; //25;
