...
let DEBUG = localStorage.getItem('DEBUG') === 'true' && SHOW_DEBUG_BUTTON;
...
function _html_cards(cards) {
  return;
  var html = '<div style="align-items: stretch; display: flex; flex-direction: row; flex-wrap: nowrap; overflow-x: auto; overflow-y: hidden; gap: 0rem;">';
  const cardcss = "max-width: 20%; padding: 2px; margin-bottom: 2rem; border: 0; flex-basis: 20%; flex-grow: 0; flex-shrink: 0; box-sizing: border-box; margin-top: 0.5rem; background-color: white; border: 1px solid rgb(0,0,0,0.15); border-radius: .25rem;";
...
function html_cards(cards) {
  console.log('html_cards WTF!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
  return;
  var html = '<div>';// style="align-items: stretch; display: flex; flex-direction: row; flex-wrap: nowrap; overflow-x: auto; overflow-y: hidden; gap: 0rem;">';
  const cardcss = "margin-bottom: 2rem; border: 0; flex-basis: 20%; flex-grow: 0; flex-shrink: 0; box-sizing: border-box; margin-top: 0.5rem; background-color: white; border: 1px solid rgb(0,0,0,0.15); border-radius: .25rem;"; //" "max-width: 20%; padding: 2px; margin-bottom: 2rem; border: 0; flex-basis: 20%; flex-grow: 0; flex-shrink: 0; box-sizing: border-box; margin-top: 0.5rem; background-color: white; border: 1px solid rgb(0,0,0,0.15); border-radius: .25rem;";
  for (i = 0; i < cards.length; i++) {
    if (false && cards[i].description) {
      // html += `<div style="width: 18rem; margin: 10px;">
      html += `<div style="${cardcss}">
...
async function renderSurvey() {
  ...
  survey.onCurrentPageChanged.add(function (sender, options) {
    ...
    // console.log(`next page ${sender.currentPage.name}`);
    ...
    console.log('___________',sender.currentPage.name)
    if (sender.currentPage.name.endsWith("_selection")) {
      var ident = sender.currentPage.name.split("_").slice(0, -1).join("_");
      var selectedCategories = survey.getValue(`${ident}_categories`).map(x => categories[x]);

      //console.log('.....',sender.currentPage.getElementByName(`${ident}_candidates`))
    ...
    // if (sender.currentPage.name.endsWith("_revision")) {
    //     var ident = sender.currentPage.name.split("_").slice(0, -1).join("_");
    //     // survey.setValue(`${ident}_updated_criterion`, survey.getValue(`${ident}_criterion`));

    //     var q = survey.getPageByName(`${ident}_revision`).getElementByName(`${ident}_prev_selection`);


    //     var selection = survey.getPageByName(`${ident}_selection`).getElementByName(`${ident}_candidates`);


    //     // TODO: make sure not to duplicate the items and instead only copy the values
    //     // fill_cards(q, survey.item_candidates);
    //     q.html = fill_html_cards(selection.elements, survey.item_candidates1);
    // }
