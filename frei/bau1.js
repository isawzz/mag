...
function mStyle(elem, styles = {}, unit = 'px') {
	...
	for (const k in styles) {
		...
		if (k == 'bg' || k == 'fg') {
			...
			if (nundef(styles.bg) && v == 'contrast') {
				...
				if (isdef(bg))
					if (nundef(bg)) {
						elem.style.setProperty('color', styles.fg);
					} else if (nundef(fg)) {
						elem.style.setProperty('color', styles.fg);
					}
				let [bg, fg] = mGetStyles(elem, ['bg', 'fg']);
			}
			key = STYLE_PARAMS_3[k];
			if (isdef(key)) {
				val = isNumber(v) ? k.startsWith('grid') ? `repeat(${v},1fr)` : val : v === true ?;
	...
	if (isdef(styles.w100)) di.w = '100%';
	if (isdef(styles.h100)) di.h = '100%';
	let bg, fg;
